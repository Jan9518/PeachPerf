package com.peach.perf.library.shell;

import android.content.Context;
import android.util.Log;

import com.peach.perf.common.shell.KeepShellPublic;
import com.peach.perf.common.shell.KernelProrp;
import com.peach.perf.model.ProcessInfo;
import com.peach.perf.shell_utils.ToyboxIntaller;
    ...
    */
    ...
    private ProcessInfo readRow(String row) {
        ...
        if (columns.length >= 6) {
            try {
                ...
                // Log.e("御史中丞-ProcessUtils", "" + ex.getMessage() + " -> " + row);
            }
        } else {
            // Log.e("御史中丞-ProcessUtils", "" + row);
        }
        return null;
    }
    ...
    public ProcessInfo getProcessDetail(int pid) {
        if (DETAIL_COMMAND != null) {
            ...
            Log.d("御史中丞-SWAP", DETAIL_COMMAND + pid);
            Log.d("御史中丞-SWAP", "" + r);
            String[] rows = r.split("\n");
            if (rows.length > 1) {
